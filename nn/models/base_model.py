...
class BaseModel(nn.Module, ABC):
    ...
    def loss(self, inputs, labels: torch.IntTensor) -> torch.FloatTensor:
        # return self.loss_func(self(inputs), torch.max(labels, 1)[1])
        return torch.nn.CrossEntropyLoss()(self(inputs), torch.max(labels, 1)[1])
    ...
    def evaluate(self, test_loader, mode="sum"):
        ...
        # preds = np.zeros((3287,64))
        # true = np.zeros((3287,64))
        # c = 0
        for inputs1,inputs2,inputs3, labels in test_loader:
            ...
            # print('point2')
            # print(inputs1.shape,inputs2.shape,inputs3.shape)
            outputs = self(inputs1,inputs2,inputs3)
            # print('point3')
            write_pred = outputs.cpu().detach().numpy()
            write_true = labels.cpu().detach().numpy()
            # array_range1 = c*32
            # array_range2 = c*32 + write_pred.shape[0]
            # c+=1
            # preds[array_range1:array_range2,:] = write_pred
            # true[array_range1:array_range2,:] = write_true
            # print("outputs, labels",outputs[0], labels[0],sum(outputs[0]),outputs.shape, labels.shape)
            # batch_loss = self.loss_func(outputs, torch.max(labels, 1)[1])
            batch_loss = torch.nn.CrossEntropyLoss()(outputs, torch.max(labels, 1)[1])

            test_loss += batch_loss.item()

            labels_predicted = torch.argmax(outputs, dim=1)
            # print('labels_predicted',labels_predicted[0])
            if labels.dim() == 2:
                # print('TRue')
                labels = torch.argmax(labels, dim=1)
                # print('labels',labels)
            # print('cjjjj',torch.eq(labels_predicted[0], labels[0]))
        ...
        if mode == "mean":
            test_loss /= n_total
        self.train()
        # np.save('/home/batool/PruneFL/results/CIFAR10/centerlized_all/preds_test.npy',preds)
        # np.save('/home/batool/PruneFL/results/CIFAR10/centerlized_all/preds_true.npy',true)
