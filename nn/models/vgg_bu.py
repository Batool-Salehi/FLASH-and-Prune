...
class VGG11(BaseModel):
    def __init__(self, dict_module: dict = None):
        if dict_module is None:
            ...
            classifier = DenseSequential(DenseLinear(5876, 1024, a=0),  #800, 3200
                                         nn.ReLU(inplace=True),
                                         nn.Dropout(p=0.2, inplace=False),
                                         DenseLinear(1024, 256, a=0),
                                         nn.ReLU(inplace=True),
                                         nn.Dropout(p=0.2, inplace=False),
                                         DenseLinear(256, 64, a=1.5, mode="fan_out")
                                         # ,nn.Softmax(dim=1)
                                         )
    ...
    def _make_feature_layers_img(self):
        ...
        in_channels = 90  ###
        print('********Model is intialized*************')
        layers.extend([DenseConv2d(in_channels, 32, kernel_size=7, padding=2),nn.ReLU(inplace=True)])
        layers.extend([DenseConv2d(32, 32, kernel_size=3, padding=1),nn.ReLU(inplace=True)])
        layers.extend([DenseConv2d(32, 32, kernel_size=3, padding=1),nn.ReLU(inplace=True)])
        # layers.append(nn.MaxPool2d(kernel_size=2))
        # layers.append(nn.Dropout(p=0.3, inplace=True))
    ...
    def _make_feature_layers_gps(self):
        ...
        in_channels = 2
        print('********Model is intialized*************')
        layers.extend([DenseConv2d(in_channels, 32, kernel_size=2, padding=1),nn.ReLU(inplace=True)])
        layers.extend([DenseConv2d(32, 20, kernel_size=2, padding=1),nn.ReLU(inplace=True)])
        layers.append(nn.MaxPool2d(kernel_size=2))
        layers.append(nn.Dropout(p=0.3, inplace=True))
