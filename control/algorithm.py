...
def process_layer(layer, layer_prefix, sgrad: dict, coeff, dtp) -> (float, float, list, torch.Tensor):
    ...
    # print('total_sqg, total_time, sorted_tba_values.tolist(), sorted_tba_indices',total_sqg, total_time,sorted_tba_values.tolist()[:10],sorted_tba_indices[:10],len(sorted_tba_values.tolist()),len(sorted_tba_indices))
...
def main_control(model, squared_grad_dict: dict, config, dec_thr_pct, max_density=None):
    ...
    for layer, layer_prefix in zip(model.param_layers, model.param_layer_prefixes):
        if layer_prefix in model.prunable_layer_prefixes:
            # print('checkpoint',comm_coeff,comp_coeff_iter,next(comp_coeff_iter))
    ...
    print("\tProcessing layers Batool, time = {}.".format(timer() - proc_start))
    nas_start = timer()
