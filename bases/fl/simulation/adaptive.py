...
class AdaptiveServer(ABC):
    ...
    def initial_pruning(self, list_est_time, list_loss, list_acc, list_model_size):
        ...
        server_inputs1, server_inputs2,server_inputs3, server_outputs = [], [],[], []
        dev = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        for _ in range(self.config.IP_DATA_BATCH):
            inp1,inp2,inp3, out = self.ip_train_loader.get_next_batch()
            print('inp, out',inp1.shape, inp2.shape,inp3.shape,out.shape)
            server_inputs1.append(inp1.to(dev))
            server_inputs2.append(inp2.to(dev))
            server_inputs3.append(inp3.to(dev))
            server_outputs.append(out.to(dev))
        print('server_inputs,server_outputs',len(server_inputs1),len(server_inputs2),len(server_inputs3),len(server_outputs)) #server_inputs,server_outputs 32 32
        prev_density = None
        prev_num = 5
        prev_ind = []
        start = timer()
        ...
        for server_i in range(1, self.config.IP_MAX_ROUNDS + 1):
            ...
            if (server_i - 1) % self.config.EVAL_DISP_INTERVAL == 0:
                ...
                loss, acc = self.model.evaluate(self.ip_test_loader,mode="mean")
                # print('error line')
                train_loss, train_acc = self.model.evaluate(zip(server_inputs1, server_inputs2,server_inputs3,server_outputs),mode="mean")
            ...
            for server_inp1, server_inp2,server_inp3,server_out in zip(server_inputs1, server_inputs2,server_inputs3, server_outputs):
                list_grad = self.ip_optimizer_wrapper.step(server_inp1, server_inp2,server_inp3, server_out)    #trainig happens here
                # print('self.model.named_parameters(), list_grad',self.model.named_parameters(), list_grad)
    ...
    def main(self, idx, list_sd, list_num_proc, lr, list_accumulated_sgrad, start, list_loss, list_acc, list_est_time,
             list_model_size, is_adj_round, density_limit=None):    #round_number,model_wrights,data_per_client,
        total_num_proc = sum(list_num_proc)
        print('list_num_proc',list_num_proc)
        #averaging part
        print('**************Averaging models from clients old*******************')
        with torch.no_grad():
            for key, param in self.model.state_dict().items():
                print('*****************New one******************')
                # print('key and param',key,param)
                avg_inc_val = None
                for num_proc, state_dict in zip(list_num_proc, list_sd):
                    # print('zip(list_num_proc, list_sd)',zip(list_num_proc, list_sd))
                    if key in state_dict.keys():
                        mask = self.model.get_mask_by_name(key)
                        if mask is None:
                            # print('mask is none')
                            inc_val = state_dict[key] - param
                            # print('check',key,state_dict[key],param)
                        else:
                        ...
                        if avg_inc_val is None:
                            # print('avg_inc_val is none')
                            # print('inc_val',inc_val)
                            avg_inc_val = num_proc / total_num_proc * inc_val
                            # print('inc_val after',inc_val)

                        else:
                            # print('avg_inc_val is not none')
                            # print('inc_val',inc_val)
                            avg_inc_val += num_proc / total_num_proc * inc_val
                            # print('inc_val after',inc_val)
                ...
                if avg_inc_val is None or key.endswith("num_batches_tracked"):
                    ...
                    # print("else",param,avg_inc_val,param.add_(avg_inc_val))
                    param.add_(avg_inc_val)

        # print('list_sd',list_sd)
        # print('**************Averaging models from clients marmar*******************')
        # with torch.no_grad():
        #     for key, param in self.model.state_dict().items(): #old model
        #         # print('key',key)
        #         sum_layer_matrix = None
        #         sum_layer_matrix = 0*param
        #         print('*****************New one******************')
        #         for num_proc, state_dict in zip(list_num_proc, list_sd):  # goes over clients
        #             if key in state_dict.keys():
        #                 # print('key should be 10 times',key)
        #                 mask = self.model.get_mask_by_name(key)
        #                 if mask is None:
        #                     inc_val = state_dict[key]
        #                 else:
        #                     inc_val = state_dict[key] * self.model.get_mask_by_name(key)
        #             # print('inc_val,sum_layer_matrix',inc_val,sum_layer_matrix)
        #             sum_layer_matrix += inc_val

        #         self.model.state_dict()[key] = sum_layer_matrix



        if idx % self.config.EVAL_DISP_INTERVAL == 0:
            print('######Checkpoint########',idx,self.config.EVAL_DISP_INTERVAL)
            loss, acc = self.model.evaluate(self.test_loader,mode='mean')
            list_loss.append(loss)
            list_acc.append(acc)
            print('****************test accuracy after Fedave****************')
...
class AdaptiveClient:
    ...
    def main(self, is_adj_round):
        ...
        for ep in range(self.config.NUM_LOCAL_UPDATES):
            num_proc_data = 0
            ...
            for train_batch1, train_batch2,train_batch3, train_labels in tqdm(self.train_loader):
                ...
                # print('train_batch, train_labels',train_batch1.shape,train_batch2.shape, train_batch3.shape,train_labels.shape)
                # self.model.eval()
                outputs = self.model(train_batch1,train_batch2,train_batch3)
                # print('check outpiuts',outputs)
                # self.model.eval()
                local_train_loss = binary_cross_entropy_with_logits(outputs, train_labels)
                # print('local_train_loss',local_train_loss)
                list_grad = self.optimizer_wrapper.step(train_batch1,train_batch2,train_batch3, train_labels)
                # Getting the Acc
            ...
            local_test_loss, local_test_acc = self.model.evaluate(self.test_loader,mode='mean')

            if (ep % 1 == 0): print('epoch {}, local train loss {} local train acc {} local test loss {} local test acc {}'.format(ep, local_train_loss.data, (100 * train_correct / train_total),local_test_loss,100*local_test_acc))
            # ################ old
            ...
            # [print(i.shape) for i in list_grad]
            # ################

            print('num_proc_data',num_proc_data)
            # print('self.model.named_parameters(), list_grad',self.model.named_parameters(), list_grad)
            for (key, param), g in zip(self.model.named_parameters(), list_grad):
        ...
        if self.use_adaptive and is_adj_round:
            ...
            sgrad_to_upload = {}
        # print('self.model.state_dict()',self.model.state_dict())
...
class AdaptiveFL(ABC):
    ...
    def main(self):
        ...
        for idx in range(self.max_round):
            ...
            for client_index,client in enumerate(self.client_list):
                print('*****************Local training for clinet'+str(client_index)+'******************')
    ...
    #         ################ Added
    #         # Getting the Acc
    #         input_epoch = inputs.to(self.device)
    #         labels_epoch = labels.to(self.device)

    #         outputs_epoch = self.model(input_epoch)
    #         loss_epoch = criterion(outputs_epoch, torch.max(labels_epoch, 1)[1])

    #         outputs_epoch = outputs_epoch.cpu().detach().numpy()
    #         labels_epoch = labels_epoch.cpu().detach().numpy()
    #         train_total += labels_epoch.shape[0]
    #         train_correct += (np.argmax(labels_epoch, axis=1) == np.argmax(outputs_epoch, axis=1)).sum().item()
    #         # if (ep % 19 == 0): print('epoch {}, loss {} train acc {}'.format(ep, loss_epoch.data, (100 * train_correct / train_total)))
    #         ################
