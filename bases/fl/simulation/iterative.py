...
class ItPrServer(ABC):
    def __init__(self, args, config, model, save_interval=50):
        ...
        adaptive_folder = "adaptive_cs" if args.client_selection else "adaptive_them_all_15"
        # print('check point prune rate',adaptive_folder,load("results/{}/{}/model.pt".format(config.EXP_NAME, adaptive_folder)).density())
        # self.prune_rate = 1 - load("results/{}/{}/model.pt".format(config.EXP_NAME, adaptive_folder)).density() ** (
        #         1 / config.NUM_ITERATIVE_PRUNING)
        # self.prune_rate = 1 - 0.6570325174274417 ** (
        #         1 / config.NUM_ITERATIVE_PRUNING)
        # self.prune_rate =  0.6570325174274417/9
        self.prune_rate =  (1-0.6570325174274417)/9
...
class ItPrFL(ABC):
    def __init__(self, config, server, client_list):
        ...
        self.pruning_interval = 15     #int(config.MAX_ROUND / 2 / self.num_prunes)
